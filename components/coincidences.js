                ...
                if (data.length > 0) {
                    data.forEach(el => {
                        result += '<div class="coincidences_check">';
                            ...
                            result += `<div class="coincidences_img"><img src="../refs/arrow.png"></div>`;
                                    ...
                                    el.forEach(element => {
                                        result += `
                                            <div class="coincidences_item">
                                                ...
                                                <div>
                                                    <button class="changeDataForCheck" idElement="${element.id}" coin="${i}" type="${j}">Изменить</button>
                                                </div>
                                            </div>
                                        `
                                        j++;
                    ...
                    $(".changeDataForCheck").click(function(){
                        let idElement = $(this).attr('idElement');
                        let coinElement = $(this).attr('coin');
                        let typeElement = $(this).attr('type');
                        
                        let institute = $(`.coin_${coinElement}_institute_type_${typeElement} input`).val(); 
                        let direction = $(`.coin_${coinElement}_direction_type_${typeElement} input`).val(); 
                        let grouptitle = $(`.coin_${coinElement}_grouptitle_type_${typeElement} input`).val(); 
                        let weekday = $(`.coin_${coinElement}_weekday_type_${typeElement} input`).val(); 
                        let number = $(`.coin_${coinElement}_number_type_${typeElement} input`).val(); 
                        let title = $(`.coin_${coinElement}_title_type_${typeElement} input`).val(); 
                        let teacher = $(`.coin_${coinElement}_teacher_type_${typeElement} input`).val(); 
                        let auditorium = $(`.coin_${coinElement}_auditorium_type_${typeElement} input`).val(); 
                        let type = $(`.coin_${coinElement}_type_type_${typeElement} input`).val(); 
                        let subgroup = $(`.coin_${coinElement}_subgroup_type_${typeElement} input`).val(); 
                        let separation = $(`.coin_${coinElement}_separation_type_${typeElement} input`).val(); 
